...
function InventoryTypeSelect({ inventoryTypes, selectedInventoryTypeId, handleInventoryTypeChange }) {
  ...
  const toggleDropdown = (event) => {
    event.stopPropagation(); // Prevent bubbling up to form submit
    setIsOpen(!isOpen);
  };
      ...
      <div className='dropdown-container'>
        ...
        {isOpen && (
          <ul id="inventoryType" className='dropdown-menu'>
            {inventoryTypes.length > 10 ? ( // Check if inventoryTypes length is greater than 10
              <div className="scrollable-menu">
                {inventoryTypes.map(type => (
                  <button key={type.inventoryTypeId} value={type.inventoryTypeId} className='dropdown-item' onClick={() => handleSelect(type.inventoryTypeId)}>
                    {type.inventoryType}
                  </button>
                ))}
              </div>
